...
func GlobalManager() *SessionManager {
	if g_GlobalSessionManager == nil {
		g_GlobalSessionManager, _ = NewSessionManager("memory", fit.Config().SessionKey, fit.Config().SessionTimeout)
	}
...
func (manager *SessionManager) SessionStart(w *fit.Response, r *fit.Request) (Session, error) {
	manager.lock.Lock()
...
func (manager *SessionManager) SessionDestroy(w *fit.Response, r *fit.Request) {
